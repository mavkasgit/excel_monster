...
' === Главная процедура создания листа ===
Public Sub CreatePlanSheet()
    Dim ws As Worksheet
...
' === Создание основной таблицы "План" ===
Private Sub CreateMainTable(ByRef ws As Worksheet)
    ...
    headers = Array( _
        ...
        "Цвет", "Подвесы, шт", "УСЛОВНЫЕ ПУНКТЫ, шт", "Ламели, шт" _
    )
    ...
    ' Отключаем перенос текста для всех строк данных
    ws.Range("A2:P" & ws.Rows.Count).WrapText = False

    ' Настройка ширины колонок
    ws.Columns(1).ColumnWidth = 9.5
    ws.Columns(2).ColumnWidth = 3.5
    ...
    ws.Columns(14).ColumnWidth = 9
    ws.Columns(15).ColumnWidth = 6
    ws.Columns(16).ColumnWidth = 9
End Sub

' === Создание справочных списков ===
Private Sub CreateReferenceLists(ByRef ws As Worksheet)
    Dim sourceColumnStart As Integer
    sourceColumnStart = 19 ' начинаем с колонки S

    ' Создаем списки и привязываем к колонкам таблицы
...
' === Универсальная функция для создания справочника и привязки его к колонке ===
Private Function CreateAndLinkReferenceList(ByRef ws As Worksheet, ByVal targetColumnLetter As String, _
                                            ByVal sourceCol As Integer, ByVal values As Variant, _
    ...
    ' --- 2. Создаем таблицу ---
    tableName = "tblRef" & Replace(values(0), " ", "")
    On Error Resume Next
    ...
    ' --- 3. Настройка проверки данных для целевого столбца ---
    ...
    ' --- 4. Ширина разделительной колонки ---
    ws.Columns(sourceCol + 1).ColumnWidth = 1

    ' --- 5. Возврат следующей свободной колонки ---
    CreateAndLinkReferenceList = sourceCol + 2
End Function

' === Создание тестовых данных для проверки таблицы ===
Private Sub CreateTestData(ByRef ws As Worksheet)
  ...
  Set newRow = tbl.ListRows.Add: newRow.Range.value = Array("*", "", "", "Клюшки (растрав)", "", "", "", "", "", "", "", "", "", 1, "", "")
    Set newRow = tbl.ListRows.Add: newRow.Range.value = Array(50, 1, 48, "Пороги", "Годный подвес", "Леонович Н.В.", 140, "РП", "ЮП-2076", "зажимы", "стандартная", "15 мкм", "черный (орг-ка)", 1, "", 22)
    Set newRow = tbl.ListRows.Add: newRow.Range.value = Array(100, 1, 98, "Пороги", "Годный подвес", "", 138, "РП", "ЮП-1875", "зажимы", "стандартная", "15 мкм", "серебро", 1, "", 36)
    Set newRow = tbl.ListRows.Add: newRow.Range.value = Array(20, "", 15, "Детали", "Годный подвес", "Леонович Н.В.", 6761, "Горкомплекс", "Урны", "оснастка", "допрастрав", "15 мкм", "с31", 1, "", 8)
    ...
    Set newRow = tbl.ListRows.Add: newRow.Range.value = Array(30, "", 22, "Фасады", "Подбор цвета", "", 111, "Заказчик А", "Профиль Z", "шпильки", "стандартная", "20 мкм", "золото", 1, "", 15)
    Set newRow = tbl.ListRows.Add: newRow.Range.value = Array(25, "", 22, "Детали", "Монтаж на проволоку", "", 222, "Заказчик Б", "Профиль Y", "проволока", "стандартная", "18 мкм", "бронза", 1, "", 10)
...
' Выравниваем первый столбец по правому краю
tbl.DataBodyRange.Columns(1).HorizontalAlignment = xlRight
End If
